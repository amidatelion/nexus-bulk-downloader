...
var DownloadCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if cmd.Flags().Changed("autoextract") {
			cfg.Config.AutoExtract = overrideExtract
		}
		if cmd.Flags().Changed("downloaddir") {
			cfg.Config.DownloadDir = overrideDir
		}
...
func init() {
	DownloadCmd.Flags().StringVarP(&overrideDir, "downloaddir", "d", "", "Override download directory")
	DownloadCmd.Flags().BoolVarP(&overrideExtract, "autoextract", "x", false, "Override autoextract setting")
}
